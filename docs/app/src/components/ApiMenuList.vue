<template>
    <mu-list class="api-menu-list" v-if="apiList!=null && apiList.length!=0">
        <mu-list-item v-for="(apiGroup, groupName) in apiList" :key="groupName" :title="groupName.toUpperCase()" toggleNested>
            ...
            <mu-list-item v-for="(api, index) in apiGroup" :key="index" :title="api.name?api.name:api.url" :describeText="api.name?api.url:''" slot="nested" inset @click="menuClick(api.url, $event);">
...
<script>
    export default {
        ...
        methods: {
            menuClick(url, event){
                app.$emit('onApiMenuClicked', {url:url, menuOffset:getElementTop(event.path[0])});
            }
        }
    }
</script>
